...
if(btnRateStoryLaptop){
    btnRateStoryLaptop.onclick = function() {
        ...
        if(number_star_rate >= 1){
            ...
            .catch(error => {
                const elmInfo = document.getElementById('text-error-laptop');
                elmInfo.textContent = 'Bạn cần đăng nhập !';
            });
        }
    }
}

if(btnRateStoryTablet){
    btnRateStoryTablet.onclick = function() {
        ...
        if(number_star_rate >= 1){
            ...
            .catch(error => {
                const elmInfo = document.getElementById('text-error-tablet');
                elmInfo.textContent = 'Bạn cần đăng nhập !';
            });
        }
    }
...
if(btnRateStoryMobile){
    btnRateStoryMobile.onclick = function() {
        ...
        if(number_star_rate >= 1){
            ...
            .catch(error => {
                const elmInfo = document.getElementById('text-error-mobile');
                elmInfo.textContent = 'Bạn cần đăng nhập !';
            });
        }
    }
...
if(btnAddListReadMobile){
    btnAddListReadMobile.onclick = function(){
        if (btnAddListReadMobile.innerHTML == 'Đánh Dấu'){
            ...
            .then(data => {
                if(data.status == 'success'){
                    btnAddListReadMobile.innerHTML = 'Đánh Dấu';
                }
            })
            .catch(error => console.log(error));
        }
...
if(btnComment){
    btnComment.onclick = function(){
        ...
        if(conten_comment == ''){
            conditionSubmit = false;
        }

        if(conten_comment.length >= 2){
            if(conten_comment[0] == ' ' && conten_comment[1] == ' '){
                conditionSubmit = false;
            }
        }
        ...
        if(conditionSubmit != false){
            ...
            .catch(error => {
                contentComment.value = '';
                contentComment.setAttribute('placeholder', 'Bạn cần đăng nhập để Bình Luận');
            });
        }
    }
}
...
function addReplyComment(comment_id, story_id, image_user_url, userBoolen){
    ...
    if(checkExistParent == false && userBoolen == 'False'){
    ...
    if(userBoolen == 'True'){
        document.querySelector('.list-reply-comment').style.marginTop = '30px';
    }
...
function paginationCom(elm, story_id, number_page, userBoolen){
                    ...
                    <div class="show-comment" id="parent-comment-${comment.id}">
                        ...
                        <div class="list-comments__item-body">
                            ...
                            <div class="comment-item__body-info">
                                ...
                                <div onclick="addReplyComment(${comment.id}, ${story_id}, '${user_profile_image}', '${userBoolen}')">
                                    <i class="fa-regular fa-comment-dots"></i>
...
function paginationCom2(elm, story_id, number_page, userBoolen){
    const apiPagination = "https://tangkinhcac.vn/pagination_comment/"
...
function paginationCom3(elm, story_id, userBoolen){
    const apiPagination = "https://tangkinhcac.vn/pagination_comment/"
...
function paginationComPrev(elm, story_id, userBoolen){
...
function paginationComNext(elm, story_id, total_pages_comment, userBoolen){
    const apiPaginationComment = "https://tangkinhcac.vn/pagination_comment/"
...
function submitChoicePageCom(story_id, userBoolen){
    const apiPagination = "https://tangkinhcac.vn/pagination_comment/"
